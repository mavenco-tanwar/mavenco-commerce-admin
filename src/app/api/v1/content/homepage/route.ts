...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    console.warn("MongoDB Homepage DB fetch warning:", err);
  }
...
export async function PUT(request: NextRequest) {
  ...
  try {
    ...
    const docToSave = {
      ...body,
      id: body.id || `homepage_${tenantSlug}`,
      tenantSlug: tenantSlug,
      tenantId: tenantSlug,
      type: "homepage",
      name: body.name || "Main Storefront Homepage",
    ...
    return NextResponse.json(
      {
        success: true,
        message: `Homepage for ${tenantSlug} ${isPublish ? "published live" : "draft saved"} in tenant DB`,
        data: docToSave,
      },
      { headers: corsHeaders() }
    );
    ...
    return NextResponse.json(
      { success: false, error: err.message || "Failed to update homepage" },
