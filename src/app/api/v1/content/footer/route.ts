...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    console.warn("MongoDB Footer fetch warning:", err);
  }
...
export async function PUT(request: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json(
      {
        success: true,
        message: `Footer for ${tenantSlug} saved in tenant DB`,
        data: docToSave,
      },
      { headers: corsHeaders() }
    );
    ...
    return NextResponse.json(
      { success: false, error: err.message || "Failed to update footer" },
